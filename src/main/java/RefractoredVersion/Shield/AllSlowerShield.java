...
public class AllSlowerShield {
    ...
    public boolean verifySafe(Action candidateAction) throws Exception {
        ...
        sequence = new FixedEvolutionSequence(toSampleSetsFromSampleTraces(sampleTraces));
    ...
    private List<List<List<Vehicle>>> predictCandidateSampleTraces(Action candidateAction) throws Exception {
    ...
    private List<List<Vehicle>> predictCandidateTrace(Action candidateAction) throws Exception {
    ...
    private void applyCandidateAction(Action candidateAction) {
        for (Vehicle vehicle : sandboxEngine.vehicles) {
            if (vehicle instanceof NonNpcVehicle) {
                applyActionToVehicle(vehicle, candidateAction);
            }
        }
    }

    private void applyActionToVehicle(Vehicle vehicle, Action action) {
        switch (action) {
            case LANE_LEFT:
                vehicle.setTargetLaneIndex(Math.max(0, vehicle.getLaneIndex() - 1));
                break;
            case LANE_RIGHT:
                vehicle.setTargetLaneIndex(Math.min(sandboxEngine.numLanes - 1, vehicle.getLaneIndex() + 1));
                break;
            case FASTER:
                vehicle.targetSpeed = clipTargetSpeed(vehicle.targetSpeed + 5);
                break;
            case SLOWER:
            ...
            case IDLE:
                break;
            default:
                throw new IllegalArgumentException("Unsupported shield candidate action: " + action);
        }
    }
