...
/**
 * Evaluates an AI action followed by a sequence of future actions using
 * evolution traces supplied by an external prediction system.
 *
 * <p>This class is not a native STARK prediction implementation. It does not
 * use a STARK {@code Controller}, an environment-update function, and a
 * {@code ControlledSystem} to generate its evolution sequence. Instead, it
 * simulates the action sequence with {@link SandboxJavaHighwayEngine}, converts
 * the resulting vehicle traces into STARK data and system states, and wraps the
 * precomputed samples in a fixed {@link EvolutionSequence} for DisTL
 * evaluation.</p>
 *
 * <p>This adapter illustrates that DisTL evaluation is not restricted to
 * evolution sequences generated by STARK controllers and environments. An
 * external prediction model may also be used, provided that its output is
 * converted into an evolution sequence with the state structure expected by
 * the DisTL formulae.</p>
 *
 * <p>For native STARK prediction, extend this class and override the prediction
 * and verification entry points, in particular {@link #getPredictionSequence(Action)}
 * and {@link #verifySafeSequence(List)}, so that the sequence is generated from
 * a STARK controller, an environment-update function, and a controlled system.
 * {@link StarkNativeShield} provides the reference implementation.</p>
 */
...
//                + closingSpeed * closingSpeed / (2.0 * CHANGE_LANE_REAR_MAX_BRAKE);
    ...
    private static final class FixedEvolutionSequence extends EvolutionSequence {
        private FixedEvolutionSequence(List<SampleSet<SystemState>> sequence) {
